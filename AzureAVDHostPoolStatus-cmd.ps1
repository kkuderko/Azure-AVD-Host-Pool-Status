...
$Users = Get-AzWvdUserSession -ResourceGroupName $ResourceGroup -HostPoolName $HostPool

$ActiveUsers = ($Users | Select-Object SessionState | select-string -pattern "Active").count
$DisconnectedUsers = ($Users | Select-Object SessionState | select-string -pattern "Disconnected").count

Clear-Host
Write-Host "AVD Host Pool Name:" $HostPool -ForegroundColor Blue
Write-Host ($Users | Select-Object UserPrincipalName, Name, SessionState | Format-Table -AutoSize | Out-String) -NoNewLine -ForegroundColor DarkGray
...
Write-Host "AVD Sessions Usage:`t$($TotalAVDSessions) out of $($CurrentSessionLimit) ($($SessionCapacityP)%)" -ForegroundColor Yellow
Write-Host "Active Users:`t`t$($ActiveUsers)" -ForegroundColor Yellow
Write-Host "Disconnected Users:`t$($DisconnectedUsers)" -ForegroundColor Yellow
Write-Host
Write-Host "AVD Hosts:`t`t" $AvailableHosts "available out of" $TotalAVDHosts -ForegroundColor $AHColour
Write-Host "Hosts in drain mode:`t" $DrainModeHosts -ForegroundColor $DHColour
Write-Host
